            ...
            try
            {
                while (true)
                {
                    ...
                    switch (message.Substring(0, 4))
                    {
                            ...
                            case "~png": {
                                await clientSocket.SendAsync(Encoding.UTF8.GetBytes("@tic\0\0\0\0\0\0\0"));
                                break;
                            }
                            case "sele": {
                                string games = message.Split(" ")[1].Split("GAMES=")[1];
                                string rooms = message.Split(" ")[2].Split("ROOMS=")[1];
                                string users = message.Split(" ")[3].Split("USERS=")[1];
                                string mesgs = message.Split(" ")[4].Split("MESGS=")[1];
                                string mesgtypes = message.Split(" ")[5].Split("MESGTYPES=")[1];
                                string stats = message.Split(" ")[6].Split("STATS=")[1];
                                string ranks = message.Split(" ")[7].Split("RANKS=")[1];
                                string usersets = message.Split(" ")[8].Split("USERSETS=")[1];
                                await clientSocket.SendAsync(Encoding.GetEncoding("ISO-8859-1").GetBytes($"sele\0\0\0\0\0\0\0\u00A5GAMES=0\tMYGAME=\"1 GAMES={games} ROOMS={rooms} USERS={users} MESGS={mesgs} MESGTYPES={mesgtypes} STATS={stats} RANKS={ranks} USERSETS={usersets.Substring(1)}\"\tUSERS=0\tROOMS=0\tUSERSETS=0\tMESGS=0\tMESGTYPES=0\tASYNC=0\tCTRL=0\tSTATS=0\tSLOTS=280\tINGAME=0\tDP=XBL2/Burnout-Jan2008/mod\0"));
